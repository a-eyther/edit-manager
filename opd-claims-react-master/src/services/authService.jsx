...
import { authenticateMockUser, shouldUseMockAuth } from '../utils/mockAuth';

/**
 * Authentication API service
 * Handles all authentication-related API calls
 * Supports mock authentication for development without backend
 */
...
const authService = {
  ...
  login: async (credentials) => {
    ...
    if (shouldUseMockAuth()) {
      ...
      const mockUser = authenticateMockUser(credentials.username, credentials.password);

      if (mockUser) {
        console.log('✅ Mock login successful:', mockUser.user.username);
        return mockUser;
      } else {
  ...
  getCurrentUser: async () => {
    ...
    if (shouldUseMockAuth()) {
      ...
      const { getMockUserProfile } = await import('../utils/mockAuth');
      return getMockUserProfile(token);
    }
