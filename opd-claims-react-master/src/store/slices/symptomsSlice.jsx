 ...
 */
const buildSymptomsKey = (symptoms = {}) => {
  const normalizedCode = (symptoms.code || '').toString().trim().toLowerCase()
  const normalizedText = (symptoms.text || '').toString().trim().toLowerCase()
...
const symptomsSlice = createSlice({
  ...
  reducers: {
    ...
    addSelectedSymptom: (state, action) => {
      const newKey= buildSymptomsKey(action.payload)
      const exists = state.selectedSymptoms.some(
          (symptoms) => buildSymptomsKey(symptoms) === newKey
      )
    ...
    removeSelectedSymptom: (state, action) => {
      const removedKey = buildSymptomsKey(action.payload)
      state.selectedSymptoms = state.selectedSymptoms.filter(
        (symptoms) => buildSymptomsKey(symptoms) !== removedKey
      )
    },
