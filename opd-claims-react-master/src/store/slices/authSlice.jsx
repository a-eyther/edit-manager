...
/**
 * Normalize the user object that comes from the API so UI code can
 * reliably access common fields regardless of backend naming.
 */
const normalizeUserData = (user) => {
  ...
  const providedFullName = user.fullName ?? user.full_name ?? '';
  const fallbackFullName = `${firstName} ${lastName}`.trim();

  let role = user.role ?? user.user_role;
  if (!role) {
  ...
  return {
    ...
    fullName: providedFullName || fallbackFullName || user.username || user.email || 'User',
    role: role || null,
  };
};
 ...
 */
const persistedUser = normalizeUserData(storageService.getUserData());
...
export const fetchCurrentUser = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const normalizedUser = normalizeUserData(apiUser);
      if (normalizedUser) {
        storageService.setUserData(normalizedUser);
      }
