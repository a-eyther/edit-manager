...
export const transformClaimExtractionData = (apiResponse) => {
  ...
  const transformedDocuments = documents && documents.length > 0
    ? documents.map(doc => ({
        ...
        type: ((doc.file_type || doc.type || 'PDF')).toString().toUpperCase(),
...
console.log("policy_details",policy_details);
  ...
  return {
    ...
    patient: {
      ...
      // relation: getValue(
      //     policy_details?.claimHistory?.[0]?.beneficiaryType,
      //     getValue(claim_details?.beneficiaryType, 'Not Available')
      // ),
      relation: (() => {
          const originalRrelationDetail = getValue(policy_details?.claimHistory?.[0]?.beneficiaryType, getValue(claim_details?.beneficiaryType, 'Not Available')
          )
          if (originalRrelationDetail?.toUpperCase() === 'PRINCIPAL') {
              return 'SELF'
          }
          return originalRrelationDetail
      })(),
    ...
    document: {
      name: documentName,
      type: documents && documents.length > 0 ? ((documents[0].file_type || documents[0].type || 'PDF')).toString().toUpperCase() : 'PDF',
